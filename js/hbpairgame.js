...
//        , music: "audio/movin-cruisin-xylo.mp3", music0: "audio/movin-crusin-cover1.mp3"
        ...
        , modes: [
            { id:1, match:'-', n1: 2, good: 8, pairs: [1, 2, 3, 4], name:"level 1" },
    ...
    function showSelectedMode(n) {
        ...
        d2.src = `${gameDesc.image.path}/${gameDesc.image.prefix}${num}${sn2}.jpg`;
    ...
    function restartGame() {
        ...
        for (var i = 0; i < N; i++) {  // add tiles
            ...
            var sn1 = (mode.match == '-') ? '' : mode.match.charAt(0);
            var sn2 = (mode.match == '-') ? '' : mode.match.charAt(1);
            gameData.targetList.push( { id: `p${1+2*i}`, pairId: i, src: `${gameDesc.image.path}/${gameDesc.image.prefix}${list[i]}${sn1}.jpg`, posid: poslist[i*2] } );
            gameData.targetList.push( { id: `p${2+2*i}`, pairId: i, src: `${gameDesc.image.path}/${gameDesc.image.prefix}${list[i]}${sn2}.jpg`, posid: poslist[i*2+1] } );
        }
        gameData.foundPairs = [];
    ...
    function showAbout() {
        ...
        for (var i = 0; i < gameDesc.modes.length; i++) {
            text.innerHTML += `- ${gameDesc.modes[i].name} mode: ${cookies.finished[i]}`
    ...
    function closeAbout() {
		...
		//if (document.getElementById('tabla').src != 'x') {
		if (gameDesc.image.tabla_w != 'x') {
